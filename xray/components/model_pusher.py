...
class ModelPusher:
    ...
    def build_and_push_bento_image(self):
        ...
        try:
            if not self.model_pusher_config.ecr_registry:
                raise ValueError(
                    "Set ECR_REGISTRY (for example, account.dkr.ecr.region.amazonaws.com) "
                    "before enabling PUSH_MODEL."
                )
            ...
            os.system(
                ...
                f"{self.model_pusher_config.bentoml_service_name}:latest -t {image}"
            )
            ...
            os.system(
                f"aws ecr get-login-password --region "
                f"{self.model_pusher_config.aws_region} | docker login "
                f"--username AWS --password-stdin "
                f"{self.model_pusher_config.ecr_registry}"
            )
